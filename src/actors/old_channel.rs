...
impl ChannelState {
    ...
    fn create_response_request(&mut self) -> CreateChatCompletionRequest {
        ...
        let max_tokens = get_chat_completion_max_tokens(
            &model,
            &self.context.to_openai_chat_history(true),
        )
        .unwrap();
    ...
    async fn execute_command(
        ...
        if command == "transcribe" {
            ...
            for url in &mut urls {
                ...
                self.send_message(chat_message.clone(), "Transcribing url".to_string());
            ...
            if let Some(model) = params {
                ...
                self.model = model;
                self.send_message(chat_message, format!("Set model to {}", self.model));
            } else {
                self.send_message(chat_message, format!("Current model is {}", self.model));
            }
        }
    }
}
    ...
    async fn handle(
        ...
        match message {
            ChannelMessage::Register(chat_message) => {
                ...
                let content = format!("QUESTION: {}", chat_message.content.clone());
                let name = state
                    .wakeword
                    .clone()
                ...
                if content.starts_with("QUESTION: !") {
